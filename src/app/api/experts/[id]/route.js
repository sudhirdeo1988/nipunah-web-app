...
/**
 * GET /api/experts/{id}
 * Proxy endpoint to fetch an expert by id (auth required via cookie token)
 */
export async function GET(request, { params }) {
  try {
    const { id } = params || {};
    const url = `${API_BASE_URL}/experts/${id}`;
    ...
    console.error("Get expert by id proxy error:", error);
...
/**
 * PUT /api/experts/{id}
 * Proxy endpoint to update an expert by id (auth required via cookie token)
 */
export async function PUT(request, { params }) {
  try {
    ...
    console.error("Update expert by id proxy error:", error);
    return NextResponse.json(
      {
        error: "Internal server error",
