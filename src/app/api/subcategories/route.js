 ...
 * Proxy endpoint for subcategories API to avoid CORS issues
 * Automatically includes bearer token from cookies
 * 
 * Query Parameters:
 * - categoryId: ID of the parent category (required)
 * - page: Page number (optional, default: 1)
 * - limit: Items per page (optional, default: 10)
 * 
 * Example: GET /api/subcategories?categoryId=15&page=1&limit=5
 */
export async function GET(request) {
  try {
      ...
      return NextResponse.json(
        {
          ...
          message: "Category ID is required to fetch subcategories",
        },
        { status: 400 }
      );
    }
    ...
    // Handle errors
    console.error("Subcategories API proxy error:", error);
    return NextResponse.json(
      {
        success: false,
 ...
 * Proxy endpoint for creating subcategories.
 *
 * Expected payload:
 * { "categoryId": number, "subcategoryName": "string" }
 */
export async function POST(request) {
  try {
    const body = await request.json();

    const url = `${API_BASE_URL}/subcategories`;
